...
import { ArrowUpCircle, Wallet, AlertCircle, CheckCircle2, X } from 'lucide-react';
...
const Withdraw = () => {
  ...
  const handleWithdraw = async () => {
    ...
    if (amtValue > balance) return setStatus({ type: 'error', msg: 'Vault balance low' });

    setLoading(true);
    setStatus({ type: '', msg: '' });
    try {
      const res = await accountService.withdraw(amtValue);
      setBalance(res.balance);
      setAmount('');
      setStatus({ type: 'success', msg: `DEBIT SUCCESSFUL: $${amtValue}` });
      ...
      <AnimatePresence>
        {status.type === 'success' && (
          ...
          >
            ...
            >
              <h1 style={{ color: 'var(--success)', fontWeight: '800', fontSize: '2rem', letterSpacing: '-1px' }}>TRANSACTION COMPLETE</h1>
              <p className="p" style={{ fontSize: '1.1rem', marginTop: '10px' }}>Your vault has been updated.</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        <div className="text-center stack" style={{ gap: '6px' }}>
          <div className="flex-center" style={{ marginBottom: '8px' }}>
            <div className="glass-panel flex-center" style={{ width: '60px', height: '60px', borderRadius: '18px', background: 'white' }}>
              <ArrowUpCircle size={28} color="var(--error)" />
            </div>
          </div>
          <h2 className="h2">Vault Debit</h2>
          <p className="p">Withdraw capital from your assets</p>
        </div>
        ...
        <div className="glass-panel" style={{ background: 'white', marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px' }}>
           <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
              <span style={{ fontSize: '0.65rem', color: 'var(--text-mute)', fontWeight: '800', letterSpacing: '0.5px' }}>AVAILABLE ASSETS</span>
